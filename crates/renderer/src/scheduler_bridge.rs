...
//! manually-provided location into the scheduling engine's `ScheduleQueryResult` for
//! one output right now. Pure logic — no Wayland/GPU/D-Bus involved; `surface.rs`
//! feeds this function's result into the crossfade/idle-wait machinery.
//!
//! The scheduling engine's `ValidatedPack::query`/`next_transition_after` *panic* if
//! called with `location: None` on a solar-anchored pack — a caller-contract
//! violation, since the scheduling engine's own validation already guarantees a
//! pack's anchor kind is known before `query()` can be called at all. But this daemon
//! *can* legitimately reach that state at runtime (a solar-anchored pack assigned to
//! an output before any location is ever configured) — calling `query()` naively
//! there would crash the whole daemon, not just degrade the one output. This module
//! checks the pack's anchor kind *before* ever calling into the scheduling engine, so
//! that condition becomes [`RendererError::LocationRequired`] (a per-output degrade)
//! instead of a panic.
...
///   `Ok(None)` — a well-defined empty state, not an error.
...
/// - Otherwise → `Ok(Some(result))`, the scheduling engine's own deterministic answer.
pub fn evaluate(
    output: &OutputId,
...
/// per-output contribution to `WallpaperDaemon::next_wake`.
///
/// `ValidatedPack::next_transition_after` has the exact same "panics if called with
/// `location: None` on a solar-anchored pack" hazard this module's own doc already
/// describes for `query`/[`evaluate`]. Calling it directly here without the same guard
/// would crash the daemon on a clean install with the (solar-anchored) starter pack
/// auto-assigned and no location configured yet, as part of the very first idle-wait
/// reschedule.
///
/// `None` both when there's no pack loaded (unassigned) or it never transitions (a
/// single-image/static pack), *and* when it's solar-anchored with no location
/// available yet: that output simply doesn't contribute a wake deadline until a
/// location becomes available, rather than panicking the whole daemon. Every location
/// change already calls `reschedule_idle_timer()` again (see its own doc), so this
/// output starts contributing as soon as one is.
...
mod tests {
    ...
    /// Regression: a solar-anchored pack with no location must not panic
    /// `next_wake_for` — it degrades to no wake-deadline contribution instead.
    /// Without this, this exact call sequence would crash `wallpaperd` on every
    /// clean install, since the starter pack is solar-anchored and there's no
