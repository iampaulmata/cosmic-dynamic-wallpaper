...
- [ ] T028 [P] [US6] In `crates/wallpaperctl/src/commands/location.rs`, replace `location set`'s
      sequential per-field `Config::set`/`write_entry` calls with a single
      `cosmic_config::Config::transaction()` covering every field; add a regression test asserting
      a simulated interrupted transaction leaves the prior state fully intact rather than
      partially updated (FR-025, research.md R20)
...
- [ ] T036 [P] [US7] In `crates/renderer/src/gpu.rs`, add `const GPU_REQUEST_TIMEOUT: Duration =
      ...
      R28)
- [ ] T037 [US7] In `crates/renderer/src/surface.rs`'s draw path, on `SurfaceError::Lost`/
