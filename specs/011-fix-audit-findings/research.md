...
### R20: Multi-field config write atomicity (FR-025)

- **Decision**: Replace `commands/location.rs`'s sequential per-field `Config::set`/`write_entry`
  calls with `cosmic_config::Config::transaction()` (confirmed present on the exact `cosmic-config`
  git revision this workspace already pins — `pub fn transaction(&self) -> ConfigTransaction<'_>`),
  committing every field of a `location set` invocation as one transaction.
- **Rationale**: This is the cleanest fix found during research — `cosmic-config` already ships the
  exact atomicity primitive this finding needs, and the code even carries an upstream `//TODO:
  apply all changes at once` marker at exactly this call site, meaning the intended fix was already
  anticipated but never implemented. No new abstraction needed; just adopting an existing API.
- **Alternatives considered**: A local write-ahead temp-file-then-rename pattern implemented by
  hand in `wallpaper-ipc` — rejected once `Config::transaction()` was confirmed to exist and cover
  this exact need; hand-rolling would duplicate what the dependency already provides.
